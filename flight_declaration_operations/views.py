...
def check_for_intersection(polygon1_coords,polygon2_coords):
    polygon1_coords=json.loads(polygon1_coords)
    upper1=polygon1_coords['volumes'][0]['volume']['altitude_upper']['value']
    lower1=polygon1_coords['volumes'][0]['volume']['altitude_lower']['value']
    upper2=polygon2_coords.volume.altitude_upper.value
    lower2=polygon2_coords.volume.altitude_lower.value
    intersect=False
    if(not (upper1<lower2 or upper2<lower1)):
        polygon1_c=polygon1_coords['volumes'][0]['volume']['outline_polygon']['vertices']
        polygon1 = Polygon([(point['lat'], point['lng']) for point in polygon1_c])
        polygon2_c = polygon2_coords.volume.outline_polygon.vertices
        polygon2 = Polygon([(point.lat, point.lng) for point in polygon2_c])
        print(polygon1,polygon2)
        intersect = polygon1.intersects(polygon2)
    return intersect
...
def set_flight_declaration(request):
    ...
    if fence_within_timelimits:
        ...
        if len(all_relevant_fences)==0:
            is_approved = 0
            declaration_state = 8
    '''
    all_relevant_declarations = []
    existing_declaration_within_timelimits = FlightDeclaration.objects.filter(
        start_datetime__lte=end_datetime, end_datetime__gte=start_datetime, is_approved=True
    ).exists()
    all_declarations=[]
    if existing_declaration_within_timelimits:
        ...
        print("all_relevant_declarations")
        print(all_relevant_declarations)
        relevant_id_set = []
        id_set=[]
        for i in all_relevant_declarations:
            relevant_id_set.append(i["flight_declaration_id"])
            flight_declaration = my_database_reader.get_flight_declaration_by_id(flight_declaration_id=i["flight_declaration_id"])
            if check_for_intersection(flight_declaration.operational_intent,parital_op_int_ref.volumes[0]):
                id_set.append(i["flight_declaration_id"])	
        my_fd_rtree_helper.clear_rtree_index()
        logger.info(
            "Flight Declaration relevant intersections checked, found {all_relevant_declarations} declarations".format(
                all_relevant_declarations=len(relevant_id_set)
            )
        )
        logger.info(
            "Flight Declaration intersections checked, found {all_declarations} declarations".format(
                all_declarations=len(id_set)
            )
        )
        all_declarations=len(id_set)
        if all_declarations:
